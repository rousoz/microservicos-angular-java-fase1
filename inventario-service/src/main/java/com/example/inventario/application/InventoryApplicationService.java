...
public class InventoryApplicationService {
    ...
    public InventoryDto.InventoryResponse updateItem(InventoryDto.InventoryRequest request) {
        ...
        InventoryItem item = repository.findById(request.productId()).orElseThrow(() -> new IllegalArgumentException("Inventário não encontrado: " + request.productId()));
    ...
    public InventoryDto.InventoryResponse getItem(Long id) {
        return repository.findById(id).map(this::toResponse).orElseThrow(() -> new IllegalArgumentException("Inventário não encontrado: " + id));
    }
